...
@section('body')
    <div class="grid mt-16">
        <form class="max-w-sm mx-auto" action="@if(isset($motClef)){{route('motcle.update',$motClef)}}@else{{route('motcle.store')}}@endif" method="post">
            ...
            <div>
                <label for="nom_motclef" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white underline decoration-red-600 decoration-2 underline-offset-2">Mot Clef :</label>
                <input type="text" name="nom_motclef" id="nom_motclef" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " value="@if(isset($motClef)){{$motClef->nom_motcle}}@endif" required />
            </div>
